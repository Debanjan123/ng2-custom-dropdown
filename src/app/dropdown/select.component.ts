...
export class SelectComponent implements ControlValueAccessor, OnChanges, OnInit {
    ...
    ngOnInit() {
        this.placeholderView = this.title;
    }

    ngOnChanges(changes: any) {
        if (changes.hasOwnProperty('items')) {
            this.updateOptionsList(changes['items'].isFirstChange());
        }
        if (changes.hasOwnProperty('noFilter')) {
            let numOptions: number = this.optionList.items.length;
    ...
    private valueChanged() {
        this._value = this.optionList.id;

        this.hasSelected = this._value.length > 0;
        this.placeholderView = this.hasSelected ? '' : this.title;
        this.updateFilterWidth();
    ...
    private updateOptionsList(firstTime: boolean) {
        ...
        if (!firstTime) {
            v = this.optionList.id;
        }

        this.optionList = new OptionList(this.items);
    ...
    private setMultipleFilterInput(value: string) {
        if (this.filterEnabled) {
            this.filterInput.nativeElement.id = value;
        }
    }
    ...
    private updateFilterWidth() {
        if (typeof this.filterInput !== 'undefined') {
            let value: string = this.filterInput.nativeElement.id;
            this.filterInputWidth = value.length === 0 ?
                1 + this.placeholderView.length * 10 : 1 + value.length * 10;
        }
    }
    /* call parent component when add new fresh data */
    private onFreshDetail(freshData) {
        this.onAdd.emit(freshData);
    }
